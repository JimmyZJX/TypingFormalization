...
This section introduces a novel algorithmic system that implements the
...
\emph{existential variables} $\al, \bt$ are introduced in the algorithmic system.
They behave similarly to unification variables,
but their scoping is restricted by the position located in the
...
\bruno{Since monotypes are identical to declarative system, maybe they
do not need to be presented again. Perhaps drop their definition from 
the syntax?} Algorithmic types include all the declarative types plus
existential variables $\al$. \bruno{Aren't terms identical to the
  Declarative system too? If they are briefly state that terms are
  identical and do not show them here. Otherwise say what is different.}

%% A \le B \mid e\Lto A \mid e\To_{a} \jg \mid \appInfAlg{A}{e}

\paragraph{Judgments and judgment chains} 
A \emph{judgment chain} $\jg$ is a core component of algorithmic
...
A$), inference ($e\To_{a} \j$) and
application inference ($\appInfAlg{A}{e}$).  Subtyping and checking are relatively simple,
since the result could be either success or fail, and no other output
is returned.  However inference and application inference return some
type that leads to further computation. We use a
continuation-passing-style encoding for them.  For example, a judgment
...
In every step reduction pops the first judgment from the worklist for processing,
possibly pushes new judgments to the worklist, and repeats this process until the list is empty.
A worklist that finally reduces to empty is considered to be successfully type-checked.
When the first element is a variable declaration, we simply continue
the process (Rules 1-3).
Note that those rules about to garbage collecting variables that are
out-of-scope for the next judgement being processed.
The other rules always deal with the first judgment in the worklist. Logically we can discern 6 group of rules.

Firstly, we have six rules (Rules 4-9) that are similar to those of
declarative subtyping.  For instance, Rule 7 consumes one subtyping
...
monotype $\tau$ with the same context, so the final solution of
$\al$, $\tau$, should be well-formed under context $\Gm$.

\paragraph{Variable scopings within worklist}
Rules 8 and 9 involve variable declaration, which demonstrates how our
...
Then reduction continues until all the subtyping judgments after $\al$
are satisfied.  Finally we can safely remove $\al$ since no occurrence
of $\al$ could leak to its left in the worklist.  On the other hand,
removing $\al$ according to the algorithm garbage-collects the
variable at the perfect time: it leaves the environment immediately
after being unreferenced.\bruno{Perhaps this is better illustrated
  with a simple example?}

The design of our judgment chain is closely related to the property of
the judgments\bruno{What property? Not very clear}.
...
\paragraph{Existential decomposition}
...
Each of the rules removes and existential variable, substitutes the
variable with its solution and continues. Importantly the rules
...
Rule 18 is similar to $\mathtt{DeclSub}$, and rewrited in the
...
Instantiation technique similar to Rules 10 and 11 applies.
