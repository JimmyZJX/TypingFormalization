...
The original DK's declarative system, shown in Figures
\ref{fig:decl:sub} and \ref{fig:decl:typing}, has a few overlapping
rules.
In contrast our algorithm removes all overlapping and makes it
clear when to apply each rule.
It is possible to create a slightly modified declarative system,
by changing a few rules in DK's declarative system, that is completely
non-overlapping as well. The changes are as follows:
\begin{enumerate}
...
In addition, we proved soundness and completeness lemmas that show that
the modified declarative system is equivalent to the original one. Thus the
``restrictions'' do not change the expressive power of the system.
...
Modifications (1) and (3) require inversion lemmas regarding rules that overlaps with them.
...
\end{lemma}
These lemmas basically state that applying the specific rules is never worse than
...
but the manual proof had several problems and we were not able to formalize it directly.
Fortunately we found a different way todo the proof and get the lemma proven.
Details about this issue can be found in Appendix~\ref{}.\bruno{reference}
...
we can find a valid instantiation $\Gm\sto\Om$ that solves all the existential variables,
such that $\Om$ also transfers all judgements successfully.
...
Some immediate corollaries, which show the soundness for specific judgement forms are: 
...
Here we refer to the non-overlapping declarative system.
...
\bruno{Tom, this is the section that I feel less helpful. Maybe
you can look at this one more closely?}

The decidability theorem is:
...
$|\cdot|_\forall$ and $|\cdot|_\to$ counts the total number of
...
\end{gather*}
\end{definition}
\bruno{The definitions look a bit intimidating. I do not have immediate suggestions,
  but perhaps Tom has suggestions on how to improve the presentation here.}

It is not difficult to see that all but two algorithmic reduction rules
decrease the group of measures.
(The result of Rule 29 could be directly reduced by Rule 28, which decreases the measures.)
The two exceptions are Rules 10 and 11. Both rules increase the number of existential variables
...
Therefore we argue that such split makes the worklist easier, by analyzing a subset of the worklist,
which we call \emph{instantiation judgments} $\Gm_i$ (defined with the help of $\Gm_\le$ for the proof).
...
During the reduction, minor changes to the worklist may occur, such as
existential variable solving or insertion and instantiation.
Nevertheless, the main structure of remains unchanged.

Shown in Figure~\ref{fig:worklist_ext},
worklist extension formally describes the minor changes to worklist during algorithmic reduction.
When the algorithm processes the worklist, the first judgment is processed,
and the rest of the worklist roughly preserves its shape.
