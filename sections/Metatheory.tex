...
%-------------------------------------------------------------------------------
\subsection{Declarative Worklist}
...
systems, we introduce the notion of declarative worklist $\Om$, define in
...
be instiantiated to the declarative worklist $\Om$, namely by appropriately instantiating
all exitential variables $\al$ in $\Gm$ with well-scoped monotypes $\tau$.
...
\hfill \framebox{$\Om \rto \Om'$} \hfill $\Om$ reduces to $\Om'$.
\begin{gather*}
\begin{aligned}
...
Figure~\ref{fig:decl:worklist} defines a declarative version $\Om \sto \Om'$ of
the algorithmic reduction rules, which relates the declarative worklists to the
declarative type system, by checking every judgement entry in the worklist with
the help of the corresponding conventional declarative judgement. Recall that
the worklist consists of an interleaving of the typing context with judgments.
By means of the auxiliary function $\|\Om\|$ we can extracts the typing context
out of it, for use in the conventional declarative judgments. What is does to
obtain the typing context is simply erase the judgment entries from the
worklist.
