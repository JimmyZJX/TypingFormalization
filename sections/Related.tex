...
Their follow-up work~\cite{DunfieldIndexed} enriches the bidirectional higher-rank system with
...
Their subtyping relation is enriched with deep skolemisation,
...
resulting in complicated implicit instantiation taken care by the unification process for the algorithm.
Although a hand-written proof is given, showing that the algorithm is sound and
complete with respect to their declarative specification, no machanized proof is provided.
...
A recent work proposed Guarded Impredicative Polymorphism~\cite{GuardedImpredicative},
...
Either algorithm simplifies a collection of unification constraints progressively in a single-step style.
...
Duplicated context in their unification judgments complicates the unification process,
