...
Normally $\Gm \vdash_{wf} (e : A)$ is defined by $\Gm \vdash_{wf} e$ and $\Gm \vdash_{wf} A$.
When $A = \all A'$, the type variable $a$ is not introduced to the context of $e$,
therefore $\Gm \vdash_{wf} e$ is not derivable.

To address the problem, one can either
1) change the well-formedness judgment by introducing the binder for terms as well;
or 2) add a syntax that explicitly bind the generic type variable.
The former approach requires to resolve overlapping cases
$(e : \all A)$ with $(e : B)$ where $B \neq \all B'$.
Therefore we choose the latter approach due to its simplicity.
...
Notice that the type-lambda syntax $\Lambda a.~e:A$ annotates the generic function $e$ with $\all A$,
...
The new syntax for expressions also affect the well-formedness judgment:
$$
\inferrule*[right=$\mathtt{wf\_}\Lambda$]
    {\Gm,a \vdash_{wf} e \quad \Gm,a \vdash_{wf} A}
    {\Gm \vdash_{wf} \Lambda a.~e:A}
$$
