...
it is not difficult to extend the algorithm with a type-directed translation.
...
the number of judgments (NOT judgment chains!) in the worklist.
Within expectation, subtyping judgments translate to type coercions (denoted by $f$),
...
And the form of our algorithmic judgment becomes:
$$\Gamma \hookrightarrow \Phi$$

We take Rule 18 as an example, rewritting small-step reduction into a relational style,
$$
...
but also replace ocurrances in the elaboration list.
If an existential variable is recycled, meaning that it is not constraint,
