...
refer to $\hat{\alpha}$. \jimmy{which may contain definition/solution of $\al$ and references to $\al$}
(Remember that later entries may refer to earlier
ones, but not vice versa.) This way $\hat{\alpha}$ does not escape its scope.

\jimmy{Do we talk about scoping control without markers as well here?\\
e.g. $\forall I$ ($e \Lto \all A$) or $\le\forall R$ ($A \le \all B$)\\
Text about garbage refers to this point!}

Due to the complicated scoping control method,
it is hard to argue the preciseness of scoping for each variable when designing algorithms.
We found a scoping issue related to the subsumption rule,
