...
Some of the rules handles simple cases with common approaches.
...
to its annotated type $A$, and requires $e$ to be checked agains $A$.
...
and changes mode from checking to synthesis: if $e$ synthesizes $A$ and $A\le B$
...
Lambda-functions is a critical component of bidirectional type system, and they require much attention.
Checking $\lam e$ against function type $A\to B$ is simple:
we check the body $e$ against $B$ under the context extended with $x:A$.
However, synthesizing a lambda-function is not easy in general,
and this \emph{predicative} declarative system is able to
synthesize to a suitable monotype $\sigma\to\tau$.
...
which firstly synthesize type $A$ for the function $e_1$.
If $A$ is a function type $B\to C$, Rule $\mathtt{Decl{\to}App}$ is applied,
by checking the argument $e_2$ against $B$ and returns type $C$.
The only other possibile kind of types that
the function $e_1$ could be synthesized is polymorphic types $\all A$.
