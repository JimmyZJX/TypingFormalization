...
The bidirectional type system has three judgments shown in Figure~\ref{fig:decl:typing}.
The checking judgment $\Psi\vdash e\Lto A$ checks $e$ against the type $A$ under the context $\Psi$.
The synthesis judgment $\Psi\vdash e\To A$ means that we can infer the type of $e$ under the context $\Psi$.
The application judgment $\Psi\vdash \appInf{A}{e}{C}$ means that if a function of type $A$
(which could be a polymorphic one) is applied to the argument $e$, then the whole application
synthesizes type $C$ as its return type.

Some of the rules handle simple cases with common approaches.
Rule $\mathtt{DeclVar}$ looks term variables up in the context.
Rules $\mathtt{Decl1I}$ and $\mathtt{Decl1I{\To}}$ checks and synthesizes the unit type, respectively.
Rule $\mathtt{DeclAnno}$ synthesizes the type of the annotated expression $(e:A)$
to its annotated type $A$, and requires $e$ to be checked against $A$.
Checking an expression $e$ against a polymorphic type $\all A$ under the context $\Psi$ succeeds
if $e$ checks against A under an extended context $(\Psi, a)$, where $a$ is picked as a free variable.

The subsumption rule $\mathtt{DeclSub}$ depends on the subtyping relation,
and changes the mode from checking to synthesis: if $e$ synthesizes $A$ and $A\le B$
($A$ is more polymorphic than $B$), then $e$ can be checked against $B$.
If a checking problem does not match any other rules,
$\mathtt{DeclSub}$ can be applied and try to synthesize a type
before checking whether the synthesized one is more general.

Lambdas are critical components of the bidirectional type system, and they require much attention.
Checking $\lam e$ against function type $A\to B$ is simple:
we check the body $e$ against $B$ under the context extended with $x:A$.
However, synthesizing a the type of an unannotated lambda is not easy. 
Nevertheless the \emph{predicative} declarative system is able to
synthesize to a suitable monotype $\sigma\to\tau$, if one exists.
In other words, if no type information is given for a lambda function,
we will synthesize it to the best mono-typed function according to the context,
which also requires some guessing similar to the subtyping rule $\mathtt{\forall L}$.

Applications $e_1~e_2$ are handled by Rule $\mathtt{Decl{\to}E}$,
which firstly synthesizes type $A$ for the function $e_1$.
If $A$ is a function type $B\to C$, Rule $\mathtt{Decl{\to}App}$ is applied,
by checking the argument $e_2$ against $B$ and returns type $C$.
The only other possibile kind of type that
the function $e_1$ could be synthesized to is a polymorphic type $\all A$.
In that case, we instantiate $A$ to $[\tau/a]A$ with a monotype $\tau$ (which is also guessed)
according to Rule $\mathtt{Decl{\to}I{\To}}$.
If $[\tau/a]A$ is a function type, Rule $\mathtt{Decl{\to}App}$ proceeds;
if $[\tau/a]A$ is still polymorphic, Rule $\mathtt{Decl{\to}I{\To}}$ is recursively applied.
