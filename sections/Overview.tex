...
refer to $\al$. \jimmy{which may contain definition/solution of $\al$ and references to $\al$}
...
which might cause existential variable leakage across judgments.
...
\bruno{I think this section needs more detail here. The textual
  description alone does not convey much about the key ideas of the
  approach. I suggest showing the forms of the judgements and maybe
  illustrating with some examples why the blend between ordered
  contexts and worklists leads to precise scope traking. This can be
  done just by illustrating a concrete instance of an algorithmic
  context (no derivation is actually needed). Similar things should be
done for inference judgements. Actually chains needs some careful
explanation, I think since they are the hardest to get aspect of the formalization.}

\jimmy{Some high level description for the structure}
A worklist is structurally a stack with its top element to the right.
$$\begin{aligned}
...
For example, the top judgment of the following worklist
checks the identity function against $\all a \to a$:
$$\Gm \Vdash \lam x \Lto \all a \to a$$
Then, two auxiliary variables $a$ and $x$ are introduced to help the type checking:
...
where the first and second judgment chains are two subtyping judgments,
and the third judgment chain, $(\lam x)~() \To_a a \le \al$,
is a sequence of an inference judgment followed by a subtyping judgment.

The algorithm first analyse the two subtyping judgments and
...
After that, we proceed the last judgment chain from the left,
